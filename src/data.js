...
const ACHS = [
  ...
  { id:'gild1',    icon:'🎖️', name:'金ピカ主義',       desc:'金箔コーティングで無敵になる' },
  { id:'decree1',  icon:'📜', name:'規制歓迎派',       desc:'規制通達でトゲを一掃する' },
...
/* ---------- 遊び方（タイトルの❓から開くオーバーレイ） ---------- */
const HELP_LINES = [
  ['🎈', 'ふくらむほど資産が増える（サイズの2乗）。当たり判定も大きくなる'],
  ['💨', '長押しでカーソルの方へ噴射移動。そのぶん風船はしぼむ'],
  ['💥', 'トゲやボスに触れたら崩壊。ふくらみ切って1秒放置でも崩壊'],
  ['✨', 'トゲすれすれで「リスクプレミアム」ボーナス＋フィーバーゲージ'],
  ['🌈', 'ゲージMAXでフィーバー：12秒間 資産3倍・配当2倍'],
  ['🪙', '🪙配当・💎大当たり・🎖️4秒無敵・📜トゲ一掃。近づくと吸える'],
  ['🌪️', '噴射の風はトゲを吹き飛ばす。ボス戦では誘導弾に変わる'],
  ['📊', '四半期（18秒）ごとに投資戦略を1つドラフト'],
  ['🐻', '第4四半期ごとにボス襲来。ベアとタカが交互に来る'],
  ['📅', '「本日の相場」は毎日同じ相場で競う日替わりモード'],
  ['🏆', '実績を集めると風船スキンが解放される'],
  ['⌨️', 'M:ミュート ／ B:BGM ／ P・Esc:ポーズ ／ V:振動 ／ Space:再上場'],
];
